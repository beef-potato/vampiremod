...
public class SpotStrengthAction extends AbstractGameAction {
    ...
    public SpotStrengthAction(int damageIncrease, AbstractMonster m) {
        this.duration = 0.0F;
    ...
    public void update() {
        if (this.targetMonster != null && this.targetMonster.getIntentBaseDmg() >= 0) {
            addToBot(new ApplyPowerAction(AbstractDungeon.player,
                    ...
                    new IntangiblePlayerPower(AbstractDungeon.player, this.damageIncrease),
                    this.damageIncrease));
        } else {
