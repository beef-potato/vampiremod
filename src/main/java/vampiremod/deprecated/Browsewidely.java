...
public class Browsewidely extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Browsewidely", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    private static final int UPG_STRENGTH_GAIN = 2;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        AbstractDungeon.actionManager.addToBottom(new TalkAction(true, Byrd.DIALOG[0],
                1.2F, 1.2F));
        addToBot( new ApplyPowerAction(p, p, new RitualPower(p, magicNumber,
                true), magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new Browsewidely();
    }
