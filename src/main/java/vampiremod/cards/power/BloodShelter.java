...
import vampiremod.powers.BloodPactPower;
import vampiremod.powers.BloodShelterPower;
import vampiremod.util.CardInfo;

import static vampiremod.vampiremod.makeID;

public class BloodShelter extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "BloodShelter", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new BloodShelterPower(p, magicNumber), magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new BloodShelter();
    }
