...
import vampiremod.powers.ConservationPower;
import vampiremod.util.CardInfo;

import static vampiremod.vampiremod.makeID;

public class Conservation extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Conservation", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new ConservationPower(p, 0), 0));
    }
