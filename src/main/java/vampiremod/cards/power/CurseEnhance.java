...
import vampiremod.powers.CurseEnhancePower;
import vampiremod.powers.MysophobiaPower;
import vampiremod.util.CardInfo;

import static vampiremod.vampiremod.makeID;

public class CurseEnhance extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "CurseEnhance", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new CurseEnhancePower(p, magicNumber), magicNumber));
    }


    @Override
    public AbstractCard makeCopy() {
        return new CurseEnhance();
    }
