...
public class Ridofevil extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Ridofevil", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    private static final int GAIN_BLOCK = 4;
    private static final int UPG_GAIN_BLOCK = 3;
    public Ridofevil() {
        super(cardInfo); //Pass the cardInfo to the BaseCard constructor.
        setMagic(GAIN_BLOCK, UPG_GAIN_BLOCK);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new LoseHPAction(p, p, HP_LOST));
        addToBot(new BlockPerNonAttackAction(magicNumber));
//        what I want to apply:
//        "DESCRIPTION": "失去 3 点生命，消耗卡组中所有状态牌",
//                "UPGRADE_DESCRIPTION": "失去 3 点生命，消耗卡组中所有状态牌和诅咒牌"
    }
    @Override
    public AbstractCard makeCopy() {
        return new Ridofevil();
    }
