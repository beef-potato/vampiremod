...
public class DeusExMachina extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "DeusExMachina", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    private static final int DRAW_OR_E = 1;
    private static final int UPG_DRAW_OR_E = 1;

    public DeusExMachina() {
        super(cardInfo); //Pass the cardInfo to the BaseCard constructor.
        setMagic(DRAW_OR_E, UPG_DRAW_OR_E);
    ...
    public void triggerWhenDrawn() {
        addToBot(new AddTemporaryHPAction(AbstractDungeon.player, AbstractDungeon.player,
                TEMPHP));
