...
import vampiremod.actions.GentlemanAction;
import vampiremod.cards.BaseCard;
import vampiremod.util.CardInfo;

import static vampiremod.vampiremod.makeID;

public class Gentleman extends BaseCard{
    private final static CardInfo cardInfo = new CardInfo(
            "Gentleman", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new GentlemanAction(STUN_AMOUNT, m));
        addToBot(new GainBlockAction(m, block));
        // test if this is the right way to apply block to enemy.
    }
