...
public class BurningBranch extends BaseCard{
    private final static CardInfo cardInfo = new CardInfo(
            "BurningBranch", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with
    ...
    private static final int UPG_CARD = 2;


    public BurningBranch() {
    ...
    public void triggerOnExhaust() {
        ...
        for (int i = 0; i<magicNumber;i ++){
            ...
            addToBot(new MakeTempCardInHandAction(AbstractDungeon.returnTrulyRandomCardInCombat().makeCopy(),
                    false));
        }

        addToBot(new MakeTempCardInHandAction((AbstractCard)new BurningBranch(), 1));

    }

    @Override
    public AbstractCard makeCopy() {
        return new BurningBranch();
    }
